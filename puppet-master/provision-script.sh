...
if [ -z "$(grep etc_puppetlabs_code_hieradata_$ENVIRONMENT /etc/fstab)" ]
then
  echo "etc_puppetlabs_code_hieradata_$ENVIRONMENT /etc/puppetlabs/code/hieradata/$ENVIRONMENT vboxsf defaults,ro 0 0" >> /etc/fstab
	mount -a
fi
